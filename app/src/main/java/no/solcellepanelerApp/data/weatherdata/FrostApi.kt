...
class FrostApi {
    ...
    val basicAuth =
        "4868c766-7477-484f-b767-6e5776a60a26:49ee1988-7461-4452-97a3-8ae5cbb133d7" // test auth
    ...
    */
    ...
    suspend fun getStations(
        ...
        var elements: String
        var polygons: MutableList<List<Pair<Double, Double>>> =
        ...
        if (requestedQuadrants.isEmpty()) {
            ...
            var currentAreas: Map<Quadrant, MutableSet<Int>> =
            ...
            requestedQuadrants.forEach { (element, quadrants) ->
                ...
                quadrants.forEach { quadrant ->
                    ...
                    intentionalSearches[element]!![quadrant] =
                        intentionalSearches[element]!![quadrant]!! + 1 // same here
    ...
    fun assignStations(
        ...
        val appendix: Map<String, Map<Quadrant, MutableList<Pair<String, Double>>>> =
            elementConst.associateWith { element ->
                Quadrant.values().associateWith { mutableListOf<Pair<String, Double>>() }
            }
        ...
        rawData.data.forEach { observationData ->
            ...
            observationData.observations.forEach { observation ->
                ...
                if (monthData.contains(month)) {
                    ...
                    stationMonthData[element]!![id]!![month] =
            ...
            quadrants.forEach { (quadrant, usableQueue) ->
                ...
                if (advancement > 0) {
                    ...
                    if (advancement <= stationQueues[element]!![quadrant]!!.size) {
                        ...
                        stationMonthData[element]!!.get(stationid)?.takeIf { it.size == 12 }
                            ?.let { // If it has at least one full year
                                usableQueue.add(stationid)
                            }
                        queueAdvancements[element]!![quadrant] =
                            (advancement * -1) // Means it has already tested this
                    }
                }
    ...
    suspend fun fetchFrostData(
        ...
        val stationMonthData: Map<String, MutableMap<String, MutableMap<Int, Pair<Double, Int>>>> =
        ...
        val stationQueues: Map<String, Map<Quadrant, MutableList<String>>> =
            elementsConst.associateWith {
                enumValues<Quadrant>().associateWith { mutableListOf() }
            } //quadrants for each element. each quadrant points to a queue of stations from the closest to the furthest. where when a station. the sorting of the queues are done before adding, not by adding
        val queueAdvancements: Map<String, MutableMap<Quadrant, Int>> =
            elementsConst.associateWith {
                enumValues<Quadrant>().associateWith { 1 }.toMutableMap()
            } // elements to quadrants to a counter of where you last dropped off in the queue. they begin at one, and all indexing require -1 on it
        val usableStations: Map<String, Map<Quadrant, MutableList<String>>> =
            elementsConst.associateWith {
                enumValues<Quadrant>().associateWith { mutableListOf() }
            } // stationQueues but only the usable stations // elements to quadrants
        val searchedAreas: Map<String, Map<Quadrant, StochasticSet>> = elementsConst.associateWith {
            enumValues<Quadrant>().associateWith { StochasticSet() }
        } // elements to quadrant to each increment searched. why the list? well, because we search with multiple elements at once, so we might pick up increment areas stochastically. why do we have this? well, because we say might want to pick up searching in certain areas when switching to extrapolation. that it does not include the areas of the initial circle works fine, but might not be intuitive and should be changed?
        val intentionalSearches: Map<String, MutableMap<Quadrant, Int>> =
            elementsConst.associateWith {
                enumValues<Quadrant>().associateWith { 1 }.toMutableMap()
            } // this is the one that modes uses to figure out whether to begin extrapolating? Set to 1 because everything gets searched once for the initial circle
        ...
        while (first == true || requestedQuadrants.isNotEmpty()) {
            ...
            while (true) {
                val response = getStations(
                    ...
                    intentionalSearches,
                    elementsConst
                ) // searches until all quadrants are filled or not worthwhile filling. puts them into the queue by proximity. the queue assumes all stations gotten after each progress is further away from center, which we will do by making the polygons circleoid. also assigns stationlocations. remember to increment intentionalsearches?
                val rawData: StationsResponse? = handleResponse(response)
                setModes(
                    modes,
                    intentionalSearches
                ) // if it has gone 250km in two directions (which means it has not found anything in them) (it can well interpolate with three quadrants, but four is optimal). if the two directions are diagonal to each other, it's also fine however. if it has found nothing in 250km in all directions, it goes to guesstimation
            ...
            while (true) {
                ...
                val response =
                    getStationData(client, missingIds, missingElements) //simply requests the data
